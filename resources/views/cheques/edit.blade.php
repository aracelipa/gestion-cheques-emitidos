...
<body>
    ...
    <div class="contenedor">
        ...
        <div class="card">
            <form action="{{ route('cheques.update', $cheque->id_cheque) }}" method="POST" class="form-principal">
                @csrf
                @method('PUT')
                ...
                <select name="id_cuenta_bancaria" id="id_cuenta_bancaria" required>
                    ...
                    @foreach($cuentas as $cuenta)
                        <option value="{{ $cuenta->id_cuenta_bancaria }}"
                            {{ old('id_cuenta_bancaria', $cheque->id_cuenta_bancaria) == $cuenta->id_cuenta_bancaria ? 'selected' : '' }}>
                ...
                <div class="fila-estado">
                    <div class="campo">
                        <select name="id_estado_actual" id="id_estado_actual" required>
                            ...
                            @foreach($estados as $estado)
                                <option value="{{ $estado->id_estado_cheque }}"
                                    {{ old('id_estado_actual', $cheque->id_estado_actual) == $estado->id_estado_cheque ? 'selected' : '' }}>
                                    {{ $estado->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @if(session('usuario_rol') === 'Supervisor')
                        <div>
                            <a href="{{ route('cheques.estado_critico', $cheque->id_cheque) }}" class="btn-edit">
                                Estado Crítico
                            </a>
                        </div>
                    @endif
                ...
                <input type="text" name="beneficiario_nombre" id="beneficiario_nombre"
                       value="{{ old('beneficiario_nombre', $cheque->beneficiario_nombre) }}" required>

                <label for="beneficiario_email">Correo del Beneficiario</label>
                <input type="email" name="beneficiario_email" id="beneficiario_email"
                       value="{{ old('beneficiario_email', $cheque->beneficiario_email) }}">

                <label for="monto">Monto (Gs.)</label>
                <input type="number" step="1" min="1" name="monto" id="monto"
                       value="{{ old('monto', (int) $cheque->monto) }}" required>

                <label for="fecha_emision">Fecha de Emisión</label>
                <input type="date" name="fecha_emision" id="fecha_emision"
                       value="{{ old('fecha_emision', \Carbon\Carbon::parse($cheque->fecha_emision)->format('Y-m-d')) }}" required>

                <label for="fecha_vencimiento">Fecha de Vencimiento</label>
                <input type="date" name="fecha_vencimiento" id="fecha_vencimiento"
                       value="{{ old('fecha_vencimiento', \Carbon\Carbon::parse($cheque->fecha_vencimiento)->format('Y-m-d')) }}" required>

                <label for="observaciones">Observaciones</label>
                <textarea name="observaciones" id="observaciones" rows="4">{{ old('observaciones', $cheque->observaciones) }}</textarea>
